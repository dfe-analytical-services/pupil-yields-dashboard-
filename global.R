...
choicesLAs <- df_py %>% filter(geographic_level=='County/Unitary') %>% pull(la_name) %>% unique() %>% sort()
choicesLADs <- df_py %>% filter(geographic_level=='District') %>% pull(la_name) %>% unique() %>% sort()

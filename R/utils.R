tidy_yields <- function(){
  ...
  dfAffordabilityHimidlow <- dfYieldsWide %>% 
    pivot_subset_combi(c("affordability","himidlow"), 
                       c("affordable.housing...high", "affordable.housing...mid", "affordable.housing...low", 
                         "market.housing...high", "market.housing...mid", "market.housing...low"))
  dfBedsAffordability <- dfYieldsWide %>% 
    pivot_subset_combi(c("number_beds","affordability"), 
                       c("X1.bed...market.housing","X2.bed...market.housing",
  ...
  dfYieldsTidy <- bind_rows(dfAfford, dfNumDevs, dfRurality, dfHousingType, dfHiMidLow,dfBeds,
...
pivot_subset <- function(df, filternames,entries,primary_filters=c("local_authority","phase_type")){
  df %>% 
    ...
    tidyr::pivot_longer(cols=-all_of(primary_filters),
...
pivot_subset_combi <- function(df, filtername,entries,primary_filters=c("local_authority","phase_type")){
  dfPivot <- df %>% select(c(primary_filters,entries)) %>%
    tidyr::pivot_longer(cols=-all_of(primary_filters),
                        names_to=filtername,
                        names_pattern="(.*)\\.\\.\\.(.*)",
                        values_to="pupil_yield")
